...
function optimize_upload_images($data) {
	...
	foreach ($files as $file) {
		...
		/* Dekodieren */
		$ysmush = json_decode(wp_remote_retrieve_body($response));

		/* Überschreiben */
		if ($ysmush && !empty($ysmush->dest)) {
			@file_put_contents(
				$path . '/' . $file,
				@file_get_contents(urldecode($ysmush->dest))
			);
		}
	}
...
add_filter('wp_generate_attachment_metadata', 'optimize_upload_images');
